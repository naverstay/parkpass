...
var CLASS_PICKED = "".concat(NAMESPACE, "-picked");
...
var helpers = {
  render: function render(type) {
    ...
    for (var i = 0; i < options.rows + 2; i += 1) {
      ...
      newValue = ~~(newValue / data.increment) * data.increment;
      if (base) {
        newValue %= base;
        if (newValue < min) {
          newValue += base;
...
var methods = {
  ...
  parseDate: function parseDate(date) {
    ...
    if (isString(date)) {
      ...
      digits = date.match(new RegExp("(".concat(groups.join('|'), ")"), 'g'));
